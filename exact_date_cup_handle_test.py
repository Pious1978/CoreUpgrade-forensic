...
from Historical_Scanner_Reconstruction import reconstruct_cup_and_handle_at_date
import sqlite3
import datetime
...
print(f"Reconstructing Cup & Handle for the exact most recent date: {target_date.date()}")

candidates = reconstruct_cup_and_handle_at_date(data, target_date)
...
live_date = conn_live.execute(
    "SELECT MAX(date) FROM setup_pivots WHERE source = 'Cup_and_Handle'"
).fetchone()[0]
live_tickers = set(row[0] for row in conn_live.execute(
    "SELECT ticker FROM setup_pivots WHERE source = 'Cup_and_Handle' AND date = ?", (live_date,)
).fetchall())
conn_live.close()
