...
class TheoremExecutor:
    ...
    def execute_suite(
        ...
        for theorem in sorted_theorems:
                ...
                try:
                    ...
                    if isinstance(raw_result, ExecutionResult):
                        sub_res = {
                            ...
                            "diagnostics": dict(raw_result.diagnostics),
                            "evidence": dict(raw_result.evidence),
                            "proof": dict(raw_result.proof),
                        }
                    ...
                    # -------------------------------------------------
                        ...
                        is_certified = bool(
                            sub_res.get("certified", False)
                        )

                        sub_res.setdefault(
                            "failure_type",
                            None if is_certified else "proof_failure",
                        )

                        sub_res.setdefault(
                            "failure_origin",
                            None if is_certified else "THEOREM",
                        )
                    ...
                    # -------------------------------------------------
                        ...
                        sub_res = {
                            ...
                            "reason": (
                                "Theorem returned unsupported response "
            ...
            # ---------------------------------------------------------
            execution_ms = round(
                (time.perf_counter() - sub_start) * 1000,
                3,
            )
            ...
            proof_data.setdefault(
                "failure_type",
                sub_res.get("failure_type"),
            )

            proof_data.setdefault(
                "failure_origin",
                sub_res.get("failure_origin"),
            )

            proof_data.setdefault(
                "severity",
                sub_res.get("severity", "ERROR"),
            )

            proof_data.setdefault(
                "reason_code",
                sub_res.get("reason_code", "UNKNOWN"),
            )
