...
from execution.capabilities.effective_capabilities import EffectiveCapabilities
...
class PaperBroker(BrokerInterface):
    ...
    def __post_init__(self) -> None:
        if self.context.broker_id != BrokerId.PAPER:
            raise ValueError(f"PaperBroker governance violation: Requires {BrokerId.PAPER}, got {self.context.broker_id}.")
    ...
    @property
    def effective_capabilities(self) -> EffectiveCapabilities:
        return EffectiveCapabilities.from_pair(
            self.capabilities,
            self.context.account_capabilities
        )
